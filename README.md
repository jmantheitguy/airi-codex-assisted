...
In addition to the original AIRI foundation, this repository includes custom work focused on practical local use:

- Local-first memory:
  short-term and long-term memory stores, decay and promotion logic, recall injection into chat, and settings pages for inspection and tuning.
- Browser-local speech:
  local Kokoro-based text-to-speech with browser defaults configured for `q4f16` and Bella (`af_bella`).
- Browser-local transcription:
  local Whisper-based speech-to-text for both recording and near-real-time chunked transcription using the selected microphone.
