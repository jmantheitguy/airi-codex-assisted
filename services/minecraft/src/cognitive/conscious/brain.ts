...
const DEFAULT_LLM_ATTEMPT_TIMEOUT_MS = 45_000
const DEFAULT_LLM_TURN_DEADLINE_MS = 120_000
...
export class Brain {
  ...
  private buildUserMessage(event: BotEvent, contextView: string): string {
    ...
    else if (event.type === 'system_alert' && event.source.id === 'autonomy') {
      const payload = event.payload as {
        reason?: string
        goal?: string
        urgency?: string
        ownerPlayerName?: string | null
        homeAnchor?: { x: number, y: number, z: number } | null
      }
      const homeText = payload.homeAnchor
        ? `home=(${payload.homeAnchor.x}, ${payload.homeAnchor.y}, ${payload.homeAnchor.z})`
        : 'home=unset'
      const ownerText = payload.ownerPlayerName ? `owner=${payload.ownerPlayerName}` : 'owner=unset'
      parts.push(`[AUTONOMY] reason=${payload.reason ?? 'unknown'} urgency=${payload.urgency ?? 'normal'} goal="${payload.goal ?? 'unspecified'}" ${homeText} ${ownerText}`)
    }
    else {
