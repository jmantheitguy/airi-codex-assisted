...
export class ToolExecutor {
  ...
  private setupHandlers(): void {
    ...
    this.disposers.push(this.debugService.onCommand('request_tools', () => {
      console.info('[ToolExecutor] Received request_tools command')
      this.sendToolsList()
    }))

    // Handle tool execution
    this.disposers.push(this.debugService.onCommand('execute_tool', (cmd) => {
...
export function setupToolExecutor(mineflayer: Mineflayer): ToolExecutor {
  if (!toolExecutorInstance) {
    toolExecutorInstance = new ToolExecutor(mineflayer)
    return toolExecutorInstance
  }

  toolExecutorInstance.setMineflayer(mineflayer)
  return toolExecutorInstance
}
