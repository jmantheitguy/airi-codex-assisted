...
import { applyPersistedMinecraftUiConfig, loadPersistedMinecraftUiConfigSync } from './ui-config'
...
export function initEnv(): void {
  ...
  const parsedConfig = configSchema.safeParse({
    ...
    bot: {
      enabled: env.BOT_ENABLED ? env.BOT_ENABLED === 'true' : defaultConfig.bot.enabled,
      ...
      version: env.BOT_VERSION?.trim() || defaultConfig.bot.version,
    },
    airi: {
  ...
  const mergedConfig = applyPersistedMinecraftUiConfig(parsedConfig.data, loadPersistedMinecraftUiConfigSync())

  config.openai = mergedConfig.openai
  config.bot = mergedConfig.bot
  config.airi = mergedConfig.airi
  config.debug = mergedConfig.debug
