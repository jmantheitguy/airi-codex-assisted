...
import { getDefaultKokoroModel, KOKORO_MODELS, kokoroFallbackVoicesToVoiceInfo, kokoroModelsToModelInfo } from '../workers/kokoro/constants'
    ...
    'browser-local-audio-speech': buildOpenAICompatibleProvider({
      ...
      createProvider: async (_config) => {
        ...
        const provider: SpeechProvider = {
          speech: () => {
            return {
              baseURL: 'http://browser-local-audio-speech/v1/',
              model: 'kokoro-82m',
              fetch: async (_input: RequestInfo | URL, init?: RequestInit) => {
                try {
                  ...
                  const selectedModel = typeof _config?.model === 'string' && _config.model
                    ? _config.model
                    : 'q4f16'
                  const modelDef = KOKORO_MODELS.find(m => m.id === selectedModel)
                  if (!modelDef) {
                    throw new Error(`Invalid browser-local model: ${selectedModel}`)
                  }
                  ...
                  console.error('Browser-local TTS generation failed:', error)
                  throw error
                }
              },
            }
      ...
      capabilities: {
        ...
        listVoices: async (config: Record<string, unknown>) => {
          void config
          return kokoroFallbackVoicesToVoiceInfo('browser-local-audio-speech')
        },
      },
      validators: {
            ...
            return {
              ...
              fetch: async (_input: RequestInfo | URL, init?: RequestInit) => {
                try {
                  ...
                  const selectedModel = typeof _config?.model === 'string' && _config.model
                    ? _config.model
                    : getDefaultKokoroModel(typeof navigator !== 'undefined' && !!navigator.gpu)
                  const modelDef = KOKORO_MODELS.find(m => m.id === selectedModel)
                  if (!modelDef) {
                    throw new Error(`Invalid Kokoro model: ${selectedModel}`)
                  }
      ...
      capabilities: {
        ...
        listVoices: async (config: Record<string, unknown>) => {
          try {
            ...
            return kokoroFallbackVoicesToVoiceInfo('kokoro-local')
          }
        },
      },
  ...
  async function validateProvider(providerId: string, options: { force?: boolean } = {}): Promise<boolean> {
    ...
    const runValidation = async () => {
      ...
      if (providerRuntimeState.value[providerId]) {
        ...
        if (validationResult.valid && ['browser-local-audio-speech', 'browser-local-audio-transcription', 'browser-web-speech-api', 'player2'].includes(providerId)) {
