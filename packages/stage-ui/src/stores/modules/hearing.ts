...
import { transcribeWithBrowserLocalWhisper } from '../providers/browser-local-transcription'
...
const STREAM_TRANSCRIPTION_EXECUTORS: Record<string, StreamTranscription> = {
  ...
  'browser-local-audio-transcription': streamBrowserLocalTranscription,
...
export const useHearingStore = defineStore('hearing-store', () => {
  ...
  // State
  const activeTranscriptionProvider = useLocalStorageManualReset('settings/hearing/active-provider', DEFAULT_HEARING_PROVIDER)
  const activeTranscriptionModel = useLocalStorageManualReset('settings/hearing/active-model', DEFAULT_HEARING_MODEL)
  ...
  const configured = computed(() => {
    ...
    // Browser-native and browser-local providers do not need a remote model selection.
    if (['browser-local-audio-transcription', 'browser-web-speech-api'].includes(activeTranscriptionProvider.value)) {
      return true
    }
    ...
    try {
      if (recording && recording.size > 0) {
        ...
        if (providerId === 'browser-web-speech-api' || providerId === 'browser-local-audio-transcription') {
          const providerConfig = providersStore.getProviderConfig(providerId) || {}
          const text = await transcribeWithBrowserLocalWhisper(
            recording,
            typeof providerConfig.language === 'string' ? providerConfig.language : 'en-US',
          )
          if (!text || !text.trim()) {
            error.value = 'No transcription result returned from local browser transcription'
            return
          }
