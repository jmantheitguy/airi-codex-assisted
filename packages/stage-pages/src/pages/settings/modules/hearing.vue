...
const providerSupportsStreaming = computed(() => supportsStreamInput.value)
const shouldUseStreamInput = computed(() => supportsStreamInput.value && !!stream.value)
const isUsingWebSpeechAPI = computed(() => activeTranscriptionProvider.value === 'browser-web-speech-api')
...
function syncBrowserLocalSettings() {
  if (activeTranscriptionProvider.value !== 'browser-local-audio-transcription')
    return

  if (!activeTranscriptionModel.value || activeTranscriptionModel.value === 'web-speech-api') {
    activeTranscriptionModel.value = 'browser-local-whisper'
  }
}
    ...
    <div flex="~ col gap-6" class="w-full md:w-[60%]">
      ...
      <div w-full rounded-xl bg="neutral-50 dark:[rgba(0,0,0,0.3)]" p-4 flex="~ col gap-4">
        ...
        <div v-else class="flex flex-col gap-4">
          <Alert v-if="isUsingWebSpeechAPI" type="warning">
            <template #title>
              Web Speech API uses your browser's default microphone
            </template>
            <template #content>
              The input meter below reflects the device selected in AIRI, but browser speech recognition does not consume that device stream directly. If speech is not transcribing, set the microphone you are speaking into as your browser or system default input.
            </template>
          </Alert>
          ...
          <div v-if="providerSupportsStreaming" class="border border-blue-200 rounded-lg bg-blue-50 p-3 dark:border-blue-800 dark:bg-blue-900/20">
            <div class="flex items-center gap-2 text-blue-700 dark:text-blue-400">
              <div i-solar:info-circle-line-duotone class="text-sm" />
              <span class="text-xs">
                {{ isUsingWebSpeechAPI
                  ? 'Streaming mode: final transcripts appear from the browser speech recognizer after you pause speaking.'
                  : 'Streaming mode: transcription starts once the microphone stream is active.' }}
              </span>
            </div>
          </div>
          ...
          <div class="space-y-3">
            ...
            <div v-if="activeTranscriptionProvider" class="text-xs text-neutral-500 dark:text-neutral-400">
              ...
              <div>Mode: <span class="font-medium">{{ providerSupportsStreaming ? (stream ? 'Streaming (real-time)' : 'Streaming capable (waiting for microphone stream)') : 'Recording (file-based)' }}</span></div>
            </div>
          </div>
        </div>
      </div>
